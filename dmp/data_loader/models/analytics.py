...
class AnalyticsDataSource(DataSource):
    ...
    upload_file = models.FileField(
        'Upload file', upload_to='file_uploads', default=None
    )
    dimensions = models.CharField(
        'Analytics dimensions', max_length=355, default=None
    )
    metrics = models.CharField(
        'Analytics metrics', max_length=355, default=None
    )
    ...
    def check_config_template_path(self):
        template_path = os.path.join(
            self.get_configs_base_path(),
            'config_templates',
            'analytics_template.yml'
        )
    ...
    def get_config_path(self, path):
        return os.path.join(
            path,
            "config_{user_id}_{data_source_id}.yml".format(
    ...
    def write_config_content(self, path, template_data):
        ...
        with codecs.open(fname, 'w', 'utf-8') as yaml_file:
            yaml_file.write(
                yaml.round_trip_dump(template_data, block_seq_indent=True)
            )
        return fname
    ...
    def get_dimensions(self):
        if self.dimensions:
            dimensions = [
                '{}'.format(dimension.strip())
                for dimension in self.dimensions.split(';')
            ]
            return dimensions
        else:
            return None
    ...
    def get_metrics(self):
        if self.metrics:
            metrics = [
                '{}'.format(metrics.strip())
                for metrics in self.metrics.split(';')
            ]
            return metrics
        else:
            return None
    ...
    def check_provider_configs_path(self):
        path = os.path.join(
            self.get_configs_base_path(),
            'providers',
            self.data_source.data_provider.name
        )
    ...
    def update_config_content(self, template_data):
        ...
        with codecs.open(os.path.join(
            settings.MEDIA_ROOT, self.upload_file.name
        ), 'r', 'utf-8') as key_file:
        ...
        if template_data:
            ...
            template_data['out']['table'] = "{}_{}_{}".format(
                self.data_source.data_provider.name,
    ...
    def create_config_file(self):
        ...
        return self.write_config_content(
            self.check_provider_configs_path(), template_content
        )

    def update_config_file(self):
        template_content = self.get_config_template_content(
            path=self.get_config_path(self.check_provider_configs_path())
        )
        template_content = self.update_config_content(template_content)
        return self.write_config_content(
            self.check_provider_configs_path(), template_content
